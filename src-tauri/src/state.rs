...
pub trait ServiceAccess {
  ...
  fn get_time<F, TResult>(&self, operation: F) -> TResult where F: FnOnce(&String) -> TResult;
  fn update_time<F, TResult>(&self, operation: F) -> TResult where F: FnOnce(&mut String) -> TResult;
}
...
impl ServiceAccess for AppHandle {
  ...
  fn get_time<F, TResult>(&self, operation: F) -> TResult 
    ...
    where F: FnOnce(&mut String) -> TResult {
        let time_state: State<TimeState> = self.state();
